...
chordLabel = Label{"Chord Recognition", x = 235, y = 75, width = 200}

piano = Piano.create("Key")
piano:setY(5)

function refreshChordLabel()
    local notes = {}
    for i, keyButton in ipairs(piano.keyButtons) do
        if keyButton.value then
            table.insert(notes, i - 1)
        end
    end
    chordLabel.text = "Chord Recognition : "
    if table.getn(notes) > 1 then
        local root, kind, bass = ChordRec.chordKind(notes)
        if kind then
            chordLabel.text = chordLabel.text..KEY[root+1]..kind
            if (root ~= bass)then
                chordLabel.text = chordLabel.text.."/"..KEY[bass+1]
            end
        else
            chordLabel.text = chordLabel.text.."Unknown"
        end
    end
end

for i, keyButton in ipairs(piano.keyButtons) do
    keyButton.changed = function(self)
        refreshChordLabel()
    end
end
